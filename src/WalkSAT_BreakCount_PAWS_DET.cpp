...
inline int WalkSAT_BreakCount_PAWS_DET::weight(int ci) const {
    return clause_weight[ci];
}



void WalkSAT_BreakCount_PAWS_DET::resetWeightStatistics(int tracked_count)
{
...
void WalkSAT_BreakCount_PAWS_DET::noteWeightIncrease(int old_weight)
{
...
void WalkSAT_BreakCount_PAWS_DET::noteWeightDecrease(int old_weight)
{
    if (old_weight <= 1) {
        return;
    }
...
WalkSAT_BreakCount_PAWS_DET::WalkSAT_BreakCount_PAWS_DET(
...
void WalkSAT_BreakCount_PAWS_DET::addUnsatClause(int ci) {
    clause_pos_in_unsat[ci] = unsat_clauses.size();
    unsat_clauses.push_back(ci);
}

void WalkSAT_BreakCount_PAWS_DET::removeUnsatClause(int ci) {
...
void WalkSAT_BreakCount_PAWS_DET::addWeightedClause(int ci) {
    clause_pos_in_weighted[ci] = weighted_clauses.size();
    weighted_clauses.push_back(ci);
}

void WalkSAT_BreakCount_PAWS_DET::removeWeightedClause(int ci) {
...
void WalkSAT_BreakCount_PAWS_DET::increaseClauseWeight(int ci) {
    ...
    if (clause_sat_count[ci] == 1 && clause_true_lit[ci] != -1)
        breakcount[clause_true_lit[ci]]++;
}

void WalkSAT_BreakCount_PAWS_DET::decreaseClauseWeight(int ci) {
    int old_w = clause_weight[ci];
...
void WalkSAT_BreakCount_PAWS_DET::randomAssignment() {
...
void WalkSAT_BreakCount_PAWS_DET::initializeClauseData() {
...
int WalkSAT_BreakCount_PAWS_DET::findUniqueTrueVar(int ci) const {
    int true_var = -1;
    int true_count = 0;
...
int WalkSAT_BreakCount_PAWS_DET::normalizeWeights()
{
    int decrements = 0;
    int i = 0;
...
int WalkSAT_BreakCount_PAWS_DET::countWeightedClauses() const
{
    return static_cast<int>(weighted_clauses.size());
}

int WalkSAT_BreakCount_PAWS_DET::maxClauseWeight() const
{
    return current_max_clause_weight;
}

double WalkSAT_BreakCount_PAWS_DET::meanClauseWeight() const
{
...
bool WalkSAT_BreakCount_PAWS_DET::solve(int max_flips, int max_tries, double p, SolverObserver* observer) {
        ...
        for (int f = 0; f < max_flips; ++f) {
            ...
            if (dist(gen) < p) {
                ...
                for (int lit : clause) {
                    ...
                    if (best_var == -1 || breakcount[v] < best_break) {
                        best_break = breakcount[v];
                        best_var = v;
                    }
                }
...
const std::vector<uint8_t>& WalkSAT_BreakCount_PAWS_DET::getAssignment() const {
    return assignment;
}
