// ============================================================
// WalkSAT_Community_MakeBreak_PAWS_DET.cpp
// ============================================================

#include "WalkSAT_Community_MakeBreak_PAWS_DET.h"
...
int WalkSAT_Community_MakeBreak_PAWS_DET::
...
double WalkSAT_Community_MakeBreak_PAWS_DET::
randDouble()
{
...
void WalkSAT_Community_MakeBreak_PAWS_DET::
randomAssignment()
{
    for (int v = 1;
         v <= num_variables;
...
// ============================================================
// Community make contributions
// ============================================================

void WalkSAT_Community_MakeBreak_PAWS_DET::
adjustCommunityMakeContribution(
    int clause,
    int delta)
{
    for (int lit : formula[clause]) {
        ...
        community_makecount[var] += delta;
    }
}

void WalkSAT_Community_MakeBreak_PAWS_DET::
addCommunityMakeContribution(
    int clause)
{
    adjustCommunityMakeContribution(
        clause,
...
void WalkSAT_Community_MakeBreak_PAWS_DET::
removeCommunityMakeContribution(
    int clause)
{
    adjustCommunityMakeContribution(
        clause,
...
inline int WalkSAT_Community_MakeBreak_PAWS_DET::
...
void WalkSAT_Community_MakeBreak_PAWS_DET::resetWeightStatistics(int tracked_count)
{
...
void WalkSAT_Community_MakeBreak_PAWS_DET::noteWeightIncrease(int old_weight)
{
...
void WalkSAT_Community_MakeBreak_PAWS_DET::noteWeightDecrease(int old_weight)
{
    if (old_weight <= 1) {
        return;
    }
...
increaseClauseWeight(int clause)
{
    ...
    if (true_literal_count[clause] == 0) {
        adjustCommunityMakeContribution(clause, +1);
    }
...
int WalkSAT_Community_MakeBreak_PAWS_DET::normalizeWeights()
{
    int decrements = 0;
    int i = 0;
...
int WalkSAT_Community_MakeBreak_PAWS_DET::countWeightedClauses() const
{
    return static_cast<int>(weighted_clauses.size());
}

int WalkSAT_Community_MakeBreak_PAWS_DET::maxClauseWeight() const
{
    return current_max_clause_weight;
}

double WalkSAT_Community_MakeBreak_PAWS_DET::meanClauseWeight() const
{
    ...
    int variable)
{
         ...
         variable_occurrences[variable])
    {
        ...
        if (is_community_clause_[clause]) {
            ...
            if (before == 0) {
                removeCommunityMakeContribution(clause);
            }
        ...
        if (is_community_clause_[clause]) {
            ...
            if (after == 0) {
                addCommunityMakeContribution(clause);
            }
    ...
    int clause)
{
    ...
    int best_score =
        community_makecount[first_var] - community_breakcount[first_var];
         ...
         ++i)
    {
        ...
        int score =
            community_makecount[var] - community_breakcount[var];
...
bool WalkSAT_Community_MakeBreak_PAWS_DET::
solve(
    int max_flips,
    int max_tries,
    double p,
