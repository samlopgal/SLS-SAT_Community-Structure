// ============================================================
// WalkSAT_Bridge_MakeBreak_PAWS_DET.cpp
// ============================================================
...
int WalkSAT_Bridge_MakeBreak_PAWS_DET::
...
double WalkSAT_Bridge_MakeBreak_PAWS_DET::
randDouble()
{
...
void WalkSAT_Bridge_MakeBreak_PAWS_DET::
randomAssignment()
{
    for (int v = 1;
         v <= num_variables;
...
// ============================================================
// Bridge make contributions
// ============================================================

void WalkSAT_Bridge_MakeBreak_PAWS_DET::
adjustBridgeMakeContribution(
    int clause,
    int delta)
{
    for (int lit : formula[clause]) {
        ...
        bridge_makecount[var] += delta;
    }
}

void WalkSAT_Bridge_MakeBreak_PAWS_DET::
addBridgeMakeContribution(
    int clause)
{
    adjustBridgeMakeContribution(
        clause,
...
initializeDataStructures()
{
    ...
    for (uint8_t is_relevant : is_bridge_clause_) {
        if (is_relevant) {
...
inline int WalkSAT_Bridge_MakeBreak_PAWS_DET::
...
void WalkSAT_Bridge_MakeBreak_PAWS_DET::resetWeightStatistics(int tracked_count)
{
...
void WalkSAT_Bridge_MakeBreak_PAWS_DET::noteWeightIncrease(int old_weight)
{
...
void WalkSAT_Bridge_MakeBreak_PAWS_DET::noteWeightDecrease(int old_weight)
{
    if (old_weight <= 1) {
        return;
    }
...
increaseClauseWeight(int clause)
{
    ...
    if (true_literal_count[clause] == 0) {
        adjustBridgeMakeContribution(clause, +1);
    }
...
int WalkSAT_Bridge_MakeBreak_PAWS_DET::normalizeWeights()
{
    int decrements = 0;
    int i = 0;
...
int WalkSAT_Bridge_MakeBreak_PAWS_DET::countWeightedClauses() const
{
    return static_cast<int>(weighted_clauses.size());
}

int WalkSAT_Bridge_MakeBreak_PAWS_DET::maxClauseWeight() const
{
    return current_max_clause_weight;
}

double WalkSAT_Bridge_MakeBreak_PAWS_DET::meanClauseWeight() const
{
    ...
    int variable)
{
         ...
         variable_occurrences[variable])
    {
        ...
        if (is_bridge_clause_[clause]) {
            ...
            if (before == 0) {
                removeBridgeMakeContribution(clause);
            }
        ...
        if (is_bridge_clause_[clause]) {
            ...
            if (after == 0) {
                addBridgeMakeContribution(clause);
            }
    ...
    int clause)
{
    ...
    int best_score =
        bridge_makecount[first_var] - bridge_breakcount[first_var];
         ...
         ++i)
    {
        ...
        int score =
            bridge_makecount[var] - bridge_breakcount[var];
...
bool WalkSAT_Bridge_MakeBreak_PAWS_DET::
solve(
    int max_flips,
    int max_tries,
    double p,
